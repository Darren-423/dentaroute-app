...
### 2026-03-20 - Edit Profile 생일 드롭다운 + 대시보드 UI 리뉴얼 + 의사 헤더 아이콘 정리
- **수정 파일**: `app/patient/edit-profile.tsx`, `app/patient/dashboard.tsx`, `app/doctor/dashboard.tsx`, `app/patient/profile.tsx`
- **변경 내용**:
  - **Edit Profile DOB**: 3열 ScrollView 리스트 → 드롭다운 버튼 3개 + 바텀시트 Modal (Year 검색 기능 포함)
  - **Patient Dashboard 헤더**: 아이콘 순서 settings → bell → dots 로 변경, 점 세개 메뉴에서 Menu Item 3 삭제
  - **Patient Dashboard 카드 리디자인**: "Clean Floating Card" - 상단 4px 상태 컬러 스트립, pill형 상태 뱃지, 단일 트랙 프로그레스 바, 연보라 border(`#ddd6e8`)
  - **Doctor Dashboard 카드 리디자인**: Patient 카드와 동일한 Clean Floating 스타일 적용 - 상단 4px 컬러 스트립, pill형 상태 뱃지, teal 톤 border(`#cddbd9`)
  - **Doctor Dashboard 헤더 아이콘**: 4개(메시지/알림/수익/설정) → 2개(수익/알림)로 축소, emoji → Feather 아이콘, 알림 뱃지를 숫자→빨간 dot으로 변경, 아이콘 배경 제거
- **영향 범위**: 환자 Edit Profile, 환자/의사 대시보드 전체
- **작업자**: Darren
