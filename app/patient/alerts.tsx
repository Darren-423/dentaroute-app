...
export default function PatientAlertsScreen() {
  ...
  return (
    <View style={s.container}>
      {/* ── Gradient header ── */}
      <LinearGradient
        colors={PatientTheme.gradient as any}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
        style={s.header}
      >
      ...
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        {notifications.length === 0 ? (
          <View style={s.emptyState}>
            <View style={[s.emptyIconWrap, { backgroundColor: PatientTheme.accentSoft, borderColor: PatientTheme.accentMid }]}>
            ...
            <View key={group.label} style={s.group}>
              ...
              {group.items.map((n) => (
                <TouchableOpacity
                  ...
                  style={[
                    s.notifCard,
                    !n.read && { backgroundColor: PatientTheme.unreadBg, borderColor: PatientTheme.unreadBorder },
                  ]}
                  onPress={() => handleTap(n)}
                  activeOpacity={0.7}
                >
                  <View style={[s.notifIconWrap, !n.read && { backgroundColor: PatientTheme.accentSoft }]}>
                    <Text style={s.notifIcon}>{n.icon}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <View style={s.notifTitleRow}>
                      <Text style={s.notifTitle} numberOfLines={1}>{n.title}</Text>
                      {!n.read && <View style={[s.unreadDot, { backgroundColor: PatientTheme.dot }]} />}
                    </View>
