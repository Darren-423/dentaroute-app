...
import { store, PatientCase, Booking } from "../../lib/store";
...
export default function PatientDeletedCasesScreen() {
  const [groups, setGroups] = useState<GroupedCases>([]);
  const [bookings, setBookings] = useState<Booking[]>([]);

  useEffect(() => {
    (async () => {
      const [cases, bks] = await Promise.all([store.getCases(), store.getBookings()]);
      setBookings(bks);
      const deleted = cases
        .filter((c) => c.hidden)
